...
local function create_order_string(params)
  local sorted_tbl ={}

  --1，使用table本身的下标遍历取值，如params[1] = "a=5"
  for i in pairs(params) do
    --params[i]的值插入到sorted_tbl中
    table.insert(sorted_tbl,i)
  end

  --2，排序，当table是这种格式{1="a=5",2="d=1"}，可以使用sort函数进行排序
  table.sort(sorted_tbl)

  --3，接下来就是把sorted_tbl中的元素拼接起来
  local sort_string = ""
  for i, v in pairs(sorted_tbl) do
    sort_string = sort_string .. v .. params[v]
  end

  return sort_string
end

local function create_hash_contenet_md5(request, hmac_params, headers)
  local secret = hmac_params.secret

  local args = ""
  local request_method = ngx_get_method()
  if "GET" == request_method then
    args = ngx_req.get_uri_args()
    args = create_order_string(args)
  elseif "POST" == request_method then
    ngx_req.read_body()
    args = ngx_req.get_body_data()
  end

  return string.upper(ngx_md5(secret..args..secret))
end

local function validate_content_md5(request, hmac_params, headers)
  local digest = create_hash_contenet_md5(request, hmac_params, headers)
  local content_md5 = headers[CONTENT_MD5]

  return digest == content_md5
end
...
--  end
  ...
  --validate md5
  if not validate_content_md5(ngx.req, hmac_params, headers) then
    return false, {status = 403, message = "HMAC signature does not match"}
  end
